...
module.exports = {
  ...
  database: false,
	execute(message, args, client) {
		...
		async function createChannel(player, timezone, offlineTime, offlineLogout, offlineLogin) { //FUNCTION NOT TESTED
			try {
			...
			if (!message.guild.channels.cache.find(c => c.name == "log" && c.type == "category")) {
                ...
                client.channels.cache.get(alertChannel.id).send(`${message.author}, your alert messages will be sent here. You should keep notifications **on** for this channel. The command \`${prefix}alert <blacklist, whitelist, language, session, offline, or version\` can individually toggle the 6 alert types, should you need to turn any of them off. Now, you should try out \`${prefix}whitelist add <game>\` so that you won't get spammed playing your favourite games! You should also try out \`${prefix}blacklist\`!`).then((msg) => msg.pin());
		...
		function writeData(player, timezone, offlineTime, offlineLogout, offlineLogin, logID, alertID) {
		  ...
		  try {
			...
			db.serialize(() => {
				...
				insertdata.run(message.author.id, message.author.tag, uuid, language, version, offlineTime, "", "", login, logout, timezone, "1 1 1 1 1 1", message.guild.id, logID, alertID, "0");
				insertdata.finalize();
				db.close();
			  });
