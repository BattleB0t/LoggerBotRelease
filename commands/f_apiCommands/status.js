      ...
      message.channel.send('Loading..').then(async msg => {
        ...
        Promise.all([
          ...
          .then((player) => {
            ...
            const timestampOfLastLogin = new Date(player[0].last_login + tzOffset).toLocaleString('en-GB', { hour12: true });
            const timestampOfLastLogout = new Date(player[0].last_login + tzOffset).toLocaleString('en-GB', { hour12: true });
        ...
        if (!player[1].online) {
            ...
            { name: 'Last Login', value: `${player[0].last_login ? `${timestampOfLastLogin}\n${timeSinceLastLogin} ago` : `Last Login: Unknown` }` },
            { name: 'Last Logout', value: `${player[0].last_logout ? `${timestampOfLastLogout}\n${timeSincefLastLogout} ago` : `Last Logout: Unknown` }` },
