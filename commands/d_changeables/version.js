...
module.exports = {
  ...
  cooldown: 10,
    ...
    async function writeNewVersion() {
      try {
        let response = await databaseImports.changeData(message.author.id, `${/[a-zA-Z]/g.test(args[0]) ? `${args[0].toUpperCase()}` : `${args[0]}`}`, `UPDATE data SET version = ? WHERE discordID = ?`);
