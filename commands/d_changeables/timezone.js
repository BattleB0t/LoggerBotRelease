...
module.exports = {
  ...
  cooldown: 10,
    ...
    async function writeNewVersion(timezone) {
      try {
        let response = await databaseImports.changeData(message.author.id, timezone, `UPDATE data SET timezone = ? WHERE discordID = ?`);
      ...
      try {
        ...
        function decimalsToUTC(decimal) {
			if (!/./.test(decimal)) {
                ...
                let hour = decimalArray[0]
                let minutes = Math.round((decimalArray[1]) * 60)
                return hour + ":" + (minutes / 100).toFixed(2).slice(2)
            } else{
          	    let hour = decimal
                return hour + ":" + "00"
            }    
